...
const wavedashAchievementIds = [
  "APPLES_30", "APPLES_80", "APPLES_140", "WALK_AROUND",
  "SURVIVE_20", "SURVIVE_40", "SURVIVE_60", "SWIM_WITH_FISHIES",
  "CHASE_15", "CHASE_40", "CHASE_70", "BOTH_CURSES", "NO_POWER_UPS", "GET_ON_ARK"
];
let wavedashReady = false;
...
function syncWavedashAchievement(index, storeNow = true) {
  const sdk = window.Wavedash;
  if (!wavedashReady || !sdk?.setAchievement) return;
  try {
    sdk.setAchievement(wavedashAchievementIds[index], storeNow);
  } catch (error) {
    console.warn("Could not sync the Wavedash achievement.", error);
  }
}

function importWavedashAchievement(index) {
  const previousBits = achievementBits;
  const previousApples = applesKnocked;
  const previousRun = longestRun;
  const previousCritters = crittersChased;

  if (index <= 2) {
    applesKnocked = Math.max(applesKnocked, [30, 80, 140][index]);
    for (let tier = 0; tier <= index; tier++) achievementBits |= 1 << tier;
  } else if (index >= 4 && index <= 6) {
    longestRun = Math.max(longestRun, [20, 40, 60][index - 4]);
    for (let tier = 4; tier <= index; tier++) achievementBits |= 1 << tier;
  } else if (index >= 8 && index <= 10) {
    crittersChased = Math.max(crittersChased, [15, 40, 70][index - 8]);
    for (let tier = 8; tier <= index; tier++) achievementBits |= 1 << tier;
  } else achievementBits |= 1 << index;

  return previousBits !== achievementBits || previousApples !== applesKnocked ||
    previousRun !== longestRun || previousCritters !== crittersChased;
}

async function initializeWavedash() {
  const sdk = window.Wavedash;
  if (!sdk) return;
  try {
    ...
    if (!sdk.requestStats || !sdk.getAchievement) return;
    const response = await sdk.requestStats();
    if (!response?.success) return;

    let imported = false;
    let needsStore = false;
    wavedashAchievementIds.forEach((id, index) => {
      const bit = 1 << index;
      if (sdk.getAchievement(id)) {
        if (importWavedashAchievement(index)) imported = true;
      } else if (achievementBits & bit) {
        syncWavedashAchievement(index, false);
        needsStore = true;
      }
    });
    if (imported) saveProgress();
    if (needsStore) sdk.storeStats?.();
  } catch (error) {
    console.warn("Wavedash SDK initialization failed; continuing with local progress.", error);
  }
}
...
window.addEventListener("keydown", event => {
  ...
  startAudio();
  if (key === "tab" && wavedashReady && window.Wavedash?.toggleOverlay) {
